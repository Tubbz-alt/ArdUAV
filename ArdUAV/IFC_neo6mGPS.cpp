...
{
	DEBUG_PORT.println(F("\tStarting NEO-6M GPS auto-configuration..."));
	...
	DEBUG_PORT.println(F("\t\tFinished NEO-6M GPS auto-configuration!"));
...
{
	DEBUG_PORT.println(F("\t\tTurning off all NMEA sentences..."));
...
{
	DEBUG_PORT.println(F("\t\tTurning on desired NMEA sentences..."));
...
{
	DEBUG_PORT.println(F("\t\tChanging baud rate..."));

	writeConfigPacket(changeBaud, BAUD_LEN);

	delay(100); // Little delay before flushing.
	GPS_PORT.flush();
	GPS_PORT.begin(115200); //reset serial port to new baud
...
{
	DEBUG_PORT.println(F("\t\tUpdating refresh frequency..."));
...
int neo6mGPS::grabData_GPS()
{
	...
	if (stringComplete_GPS)
	{
		...
		if (buffIndex == 3)
		{
			...
			//DEBUG_PORT.println(inputString_GPS);
