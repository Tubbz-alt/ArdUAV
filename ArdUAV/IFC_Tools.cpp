...
void IFC_Class::begin()
{
	...
	while (!GPS_PORT)
	{
		DEBUG_PORT.println(F("Initializing GPS port..."));
		GPS_PORT.begin(GPS_PORT_BAUD);
		delay(500);
	}
	while (!TELEM_PORT)
	...
	//initialize GPS
	DEBUG_PORT.println(F("Initializing GPS..."));
	myGPS.begin();
	DEBUG_PORT.println(F("\tGPS initialized"));




	//initialize the IMU and wait for it to "boot up"
	DEBUG_PORT.println(F("Initializing IMU..."));
	if (!bno.begin())
	{
		//there was a problem detecting the BNO055 ... check your connections
		DEBUG_PORT.println(F("\tOoops, no BNO055 detected ... Check your wiring or I2C ADDR!"));
	...
	else
	{
		...
		DEBUG_PORT.println(F("\tBNO055 successfully initialized"));
	}
	...
	DEBUG_PORT.println(F("Initializing LiDAR altimeter..."));
	myLidarLite.begin(0, true);
	myLidarLite.configure(0);
	DEBUG_PORT.println(F("\tLiDAR altimeter initialized..."));




	//initialize the servo driver
	DEBUG_PORT.println(F("Initializing servo driver and servos..."));
	...
	DEBUG_PORT.println(F("\tServo driver and servos initialized..."));




	//initialize the ESC
	DEBUG_PORT.println(F("Initializing ESC..."));
	throttle.attach(THROTTLE_PIN);
	throttle.write(THROTTLE_MIN);
	DEBUG_PORT.println(F("\tESC initialized..."));
...
void IFC_Class::sendTelem()
{
	...
	myRadio.outgoingArray[AIR_PITOT_INDEX] = (int)(telemetry.velocity * 100);
	myRadio.outgoingArray[AIR_ALTITUDE_INDEX] = (int)(telemetry.altitude * 100);
	myRadio.outgoingArray[AIR_PITCH_ANGLE_INDEX] = (int)(telemetry.pitchAngle * 100);
	myRadio.outgoingArray[AIR_ROLL_ANGLE_INDEX] = (int)(telemetry.rollAngle * 100);
	myRadio.outgoingArray[AIR_LATITUDE_INDEX] = (int)(telemetry.latitude * 100);
	myRadio.outgoingArray[AIR_LONGITUDE_INDEX] = (int)(telemetry.longitude * 100);
...
void IFC_Class::updateSingleServo(byte INDEX, uint16_t value)
{
	...
	{
		DEBUG_PORT.println(F("Servo Not recognized"));
	}
