...
------------------------------------------------------------------------------*/
LIDARLite::LIDARLite(TwoWire *i2c){
  _i2c = i2c;
}
...
{
	_i2c->begin(); // Start I2C

  if(fasti2c)
  {
    #if ARDUINO >= 157
	  _i2c->setClock(400000UL); // Set I2C frequency to 400kHz, for Arduino Due
    #else
...
{
	_i2c->beginTransmission((int)lidarliteAddress);
	_i2c->write((int)myAddress); // Set register for write
	_i2c->write((int)myValue); // Write myValue to register

  // A nack means the device is not responding, report the error over serial
  int nackCatcher = _i2c->endTransmission();
  if(nackCatcher != 0)
  {
    Serial.println("> nack");
  }
...
void LIDARLite::read(char myAddress, int numOfBytes, byte arrayToSave[2], bool monitorBusyFlag, char lidarliteAddress)
{
  ...
  while(busyFlag != 0) // Loop until device is not busy
  {
    // Read status register to check busy flag
	  _i2c->beginTransmission((int)lidarliteAddress);
	  _i2c->write(0x01); // Set the status register to be read
	...
	_i2c->requestFrom((int)lidarliteAddress,1); // Read register 0x01
    busyFlag = bitRead(_i2c->read(),0); // Assign the LSB of the status register to busyFlag
  ...
  {
	  _i2c->beginTransmission((int)lidarliteAddress);
	  _i2c->write((int)myAddress); // Set the register to be read
	...
	_i2c->requestFrom((int)lidarliteAddress, numOfBytes);
    int i = 0;
    if(numOfBytes <= _i2c->available())
    {
      while(i < numOfBytes)
      {
        arrayToSave[i] = _i2c->read();
        i++;
      }
    }
  }
